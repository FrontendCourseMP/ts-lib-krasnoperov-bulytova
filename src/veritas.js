class Veritas {
  ...
  _init() {
    if (!this._checkPrerequisites()) {
      console.warn('Veritas: Некоторые поля формы не имеют необходимых элементов (label, контейнер ошибок)');
    }
  ...
  _checkPrerequisites() {
    ...
    fields.forEach(field => {
      ...
      const hasErrorContainer = this._findErrorContainer(field);
      
      if (!hasLabel || !hasErrorContainer) {
        allValid = false;
        if (!this.options.suppressWarnings) {
          console.warn(`Veritas: Поле ${field.name} не имеет связанного label или контейнера для ошибок`);
        }
      }
    });
  ...
  _findErrorContainer(field) {
    ...
    if (containerInParent) {
      return containerInParent;
    }
  ...
  _setupLiveValidation() {
    this.form.addEventListener('blur', (e) => {
      if (e.target.matches('input, select, textarea') && this.fields.has(e.target.name)) {
  ...
  _showFieldWarnings(fieldName, warnings) {
    ...
    warnings.forEach(warning => {
      console.warn(`Veritas: ${fieldName} - ${warning}`);
    });
  }
  ...
  _validateFieldWithRules(fieldConfig, value) {
    ...
    if (!field.validity.valid) {
      ...
      if (validity.tooShort) {
        ...
        errorMessages.push(this._getErrorMessage(fieldConfig, 'minLength', `Минимальная длина: ${field.minLength}`));
      }
      if (validity.tooLong) {
        result.validity.tooLong = true;
        errorMessages.push(this._getErrorMessage(fieldConfig, 'maxLength', `Максимальная длина: ${field.maxLength}`));
      }
    ...
    for (const rule of fieldConfig.rules) {
      ...
      if (!ruleResult.isValid) {
        ...
        const errorMessage = ruleResult.errorMessage || 
  ...
  _applyRule(rule, value, field) {
    ...
    switch (rule.rule) {
      ...
      case 'equals':
        ...
        result.isValid = otherField && value === otherField.value;
        break;
  ...
  getErrors() {
    ...
    for (const [fieldName] of this.fields) {
      const result = this.validateField(fieldName);
      if (!result.isValid) {
        errors.set(fieldName, result.errorMessages);
      }
    }
